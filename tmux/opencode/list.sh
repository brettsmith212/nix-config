...
# The session of a client attached to a prefixed session (i.e. the popup we're
...
# A client NOT attached to a prefixed session — the outer client that should
...
# If we are inside a session popup, close it first.
sess="$(nested_session)"
if [ -n "$sess" ]; then
